-- Acknowledge: modify from from https://github.com/actboy168/json.lua
local type = type
local next = next
local error = error
...
if _VERSION == "Lua 5.1" or _VERSION == "Lua 5.2" then
    ...
    function utf8_char(c)
        ...
        elseif c <= 0x10ffff then
            return string_char(math_floor(c / 262144) + 240, math_floor(c % 262144 / 4096) + 128, math_floor(c % 4096 / 64) + 128, c % 64 + 128)
        end
...
function jsonc.decode(str)
    ...
    res = jsonc.flatten(res)

    setmetatable(res, jsonc)
    return res
end
