describe("variable.builtin", function()
    it("expand file", function()
        local result = require("integrator.resolver").resolve_string("${file}")
        assert.are.equal(vim.fn.expand("%:p"), result)
    end)

    it("expand relativeFile", function()
        local result = require("integrator.resolver").resolve_string("${relativeFile}")
        assert.are.equal(vim.fn.expand("%:."), result)
    end)
    it("expand file multiply", function()
        local result = require("integrator.resolver").resolve_string("${file}-${fileBasename}-${file}")
        assert.are.equal(vim.fn.expand("%:p") .. "-" .. vim.fn.expand("%:t") .. "-" .. vim.fn.expand("%:p"), result)
    end)

    it("expand workspaceFolder", function()
        local result = require("integrator.resolver").resolve_string("--${workspaceFolder}--")
...
describe("variable.env", function()
    it("expand env:HOME", function()
        local home = require("integrator.resolver").resolve_string("${env:HOME}")
        assert.are.equal(os.getenv("HOME"), home)

        local not_home = require("integrator.resolver").resolve_string("env:HOME")
        assert.are.equal("env:HOME", not_home)
    end)

    it("expand env complex", function()
        do
            local result = require("integrator.resolver").resolve_string("${env:HOME}--${env:PATH}")
            assert.are.equal(os.getenv("HOME") .. "--" .. os.getenv("PATH"), result)
        end
        do
            local result = require("integrator.resolver").resolve_string("${env:HOME}--${env:PATH}--${env:HOME}")
...
describe("variable.command", function()
    it("pickProcess", function()
        ...
        local result = require("integrator.resolver").resolve_string("${command:pickProcess}")
        -- pid is number
    ...
    it("function handle", function()
        ...
        local result = require("integrator.resolver").resolve_string("${command:test}")
    ...
    it("call once", function()
        ...
        local result = require("integrator.resolver").resolve_string("${command:test}-${command:test}")
    ...
    it("function return coroutine", function()
        ...
        local result = require("integrator.resolver").resolve_string("${command:coroutine}")
...
describe("config.resolve", function()
    it("without process", function()
        ...
        local result = require("integrator.resolver").resolve_config(config)
